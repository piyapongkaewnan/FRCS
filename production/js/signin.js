...
$('form[name=form-signin]').submit(function (event) {
    ...
    var request = $.ajax({
        ...
        url: 'checkSignIn.php', // the url where we want to POST
...
// Reset remember accout to signin other user
...
function getLocalProfile(callback) {
    ...
    var profileReRememberMe = localStorage.getItem("APPS.SITE.PROFILE_REMEMBER_ME");
    var profilelastSignIn = localStorage.getItem("APPS.SITE.PROFILE_LAST_SIGNIN");

    if (profileName !== null
            && profileReAuthUserName !== null
            && profileImgSrc !== null) {
        callback(profileImgSrc, profileName, profileReAuthUserName, profileReRememberMe, profilelastSignIn);
    }
}
...
function loadProfile() {
    ...
    getLocalProfile(function (profileImgSrc, profileName, profileReAuthUserName, profileReRememberMe, profilelastSignIn) {
        //changes in the UI
        $("#profile-img").attr("src", profileImgSrc);
        $("#profile-name").html(profileName);
        $("#reauth-username").html(profileReAuthUserName);
        $("#remember").html(profileReRememberMe);
        $("#reauth-last-signin").html(profilelastSignIn);
        $("#inputUsername").val(profileReAuthUserName);
        $("#inputUsername").hide();
        $("#remember").hide();
        $("#reauth-last-signin").show();
    });
}
 ...
 * in the first successful signin of a auth user.
...
function setLocalStorageData() {
    ...
    localStorage.setItem("APPS.SITE.PROFILE_LAST_SIGNIN", 'Last sign in : ' + dateTime);
}
...
function ResetLocalStorageData() {
    ...
    localStorage.setItem("APPS.SITE.PROFILE_LAST_SIGNIN", '');
}